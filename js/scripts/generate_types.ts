...
async function main() {
  ...
  if (process.env.SKIP_CODMOD === "1") {
    ...
    // Attempt to also write util copy if it exists
    const utilPath = path.join(SCRIPT_DIR, "../util/generated_types.ts");
    try {
      const utilCode = await fs.readFile(utilPath, "utf8");
      await fs.writeFile(utilPath, banner + utilCode);
    } catch {
      // ignore missing util
    }
  ...
  // Also attempt to fix the util copy if it exists
  const utilPath = path.join(SCRIPT_DIR, "../util/generated_types.ts");
  try {
    const utilCode = await fs.readFile(utilPath, "utf8");
    let fixedUtil = fixGenerated(utilCode);
    fixedUtil = repairRecordCalls(fixedUtil);
    if (fixedUtil !== utilCode) await fs.writeFile(utilPath, fixedUtil, "utf8");
  } catch {
    // missing util file is OK; ignore
  }
  ...
  // Ensure util copy matches the canonical generated output to avoid mismatches
  // caused by earlier post-processing iterations. Overwrite util/generated_types.ts
  // with the same content as the main generated file when present.
  const utilOut = path.join(SCRIPT_DIR, "../util/generated_types.ts");
  try {
    await fs.writeFile(utilOut, banner + code);
  } catch {
    // ignore write errors to util
  }
}
