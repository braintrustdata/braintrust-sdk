#!/bin/bash
# Script to validate release requirements for CI
# - Checks if the tag matches the version in package.json
...
# Check if tag starts with js-sdk-v
if [[ ! "$TAG" =~ ^js-sdk-v ]]; then
  echo "ERROR: Tag must start with 'js-sdk-v'"
  exit 1
fi

# Extract version without the 'js-sdk-v' prefix
