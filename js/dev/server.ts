      ...
      for (const [name, evaluator] of Object.entries(allEvaluators)) {
        ...
        if (evaluator.parameters) {
          ...
          if (Parameters.isParameters(resolvedParams)) {
            parameters = resolvedParams.serializableSchema;
            parametersSource = {
            ...
            parameters = makeEvalParametersSchema(
              resolvedParams as EvalParameters,
            );
          }
        }
    ...
    asyncHandler(async (req, res) => {
      ...
      if (evaluator.parameters) {
        try {
          ...
          if (!Parameters.isParameters(resolvedParameters) && Object.keys(resolvedParameters).length > 0) {
            validateParameters(
              parameters ?? {},
              resolvedParameters,
            );
