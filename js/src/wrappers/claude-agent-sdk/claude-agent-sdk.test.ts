...
describe.skipIf(!claudeSDK)("claude-agent-sdk integration tests", () => {
  ...
  // Regression test for issue #1655: tool spans must be children of the
  // active LLM span that emitted the tool_use block.
  test("parents a root-level tool span under the LLM span instead of the task span", async () => {
    expect(await backgroundLogger.drain()).toHaveLength(0);

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let capturedOptions: any;
    const toolUseId = "tool_use_abc123";
    ...
    const wrappedSDK = wrapClaudeAgentSDK(mockSDK);
    for await (const _msg of wrappedSDK.query({
      prompt: "Add 15 and 27 using the calculator",
    ...
    const taskSpan = spans.find(
      (s) =>
        (s.span_attributes as Record<string, unknown>).type === "task" &&
        (s.span_attributes as Record<string, unknown>).name === "Claude Agent",
    );
    const llmSpans = spans.filter(
      (s) => (s.span_attributes as Record<string, unknown>).type === "llm",
    );
    ...
    const llmSpanForToolUse = llmSpans.find((s: any) => {
    ...
    expect(llmSpanForToolUse).toBeDefined();
    expect(toolSpans[0].span_parents).not.toContain(taskSpan!.span_id);
    expect(toolSpans[0].span_parents).toContain(llmSpanForToolUse!.span_id);
  });

  test("parents a sub-agent tool span under the sub-agent LLM span instead of the task span", async () => {
    expect(await backgroundLogger.drain()).toHaveLength(0);

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let capturedOptions: any;
    const taskToolUseId = "tool_use_task_001";
    const subAgentToolUseId = "tool_use_subtool_001";
              ...
              content: [
                {
                  ...
                  id: taskToolUseId,
                  name: "Task",
                  input: { subagent_type: "math-expert" },
                },
              ],
              usage: { input_tokens: 100, output_tokens: 50 },
            },
          ...
          await postToolUseHook(
            {
              ...
              tool_name: "Task",
              tool_input: {},
              tool_response: { status: "completed", content: "42" },
              session_id: "session-1",
              transcript_path: "/tmp/transcript",
              cwd: "/tmp",
            },
    ...
    const wrappedSDK = wrapClaudeAgentSDK(mockSDK);
    for await (const _msg of wrappedSDK.query({
      prompt: "Spawn a math expert to add 15 and 27",
      options: { model: "claude-sonnet-4-20250514" },
    })) {
      // consume
    }
    ...
    const subAgentLlmSpan = llmSpans.find((s: any) => {
      const output = s.output;
      return (
        Array.isArray(output) &&
        output.some(
          (o: any) =>
            Array.isArray(o.content) &&
            o.content.some(
              (c: any) => c.type === "tool_use" && c.id === subAgentToolUseId,
            ),
        )
      );
    });
    ...
    expect(toolSpans[0].span_parents).not.toContain(subAgentSpan!.span_id);
    expect(toolSpans[0].span_parents).toContain(subAgentLlmSpan!.span_id);
  });
});
