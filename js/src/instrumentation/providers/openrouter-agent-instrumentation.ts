...
class OpenRouterAgentInstrumentationConsumer {
  ...
  private subscribeToOpenRouterAgentChannels(): void {
    traceSyncStreamChannel(openRouterAgentChannels.callModel, {
      ...
      extractInput: (args) => {
        ...
        return {
          input: request ? extractOpenRouterCallModelInput(request) : undefined,
          metadata: request
...
function getOpenRouterCallModelRequestArg(
  ...
  const keyedRequestArg = normalizedArgs.find(
    (arg) =>
      isObject(arg) && ("input" in arg || "model" in arg || "tools" in arg),
  );
  if (isObject(keyedRequestArg)) {
    return keyedRequestArg as OpenRouterAgentCallModelRequest;
  }

  const firstObjectArg = normalizedArgs.find((arg) => isObject(arg));
  return isObject(firstObjectArg)
    ? (firstObjectArg as OpenRouterAgentCallModelRequest)
    : undefined;
}
...
function serializeOpenRouterTool(
  ...
  return serialized as OpenRouterAgentTool;
}

function serializeOpenRouterToolsForLogging(
  tools: readonly OpenRouterAgentTool[] | undefined,
): OpenRouterAgentTool[] | undefined {
...
function patchOpenRouterCallModelRequestTools(
  ...
  (request as { tools?: readonly OpenRouterAgentTool[] }).tools = wrappedTools;
  return () => {
    (request as { tools?: readonly OpenRouterAgentTool[] }).tools =
      originalTools;
  };
}

function wrapOpenRouterTool(tool: OpenRouterAgentTool): OpenRouterAgentTool {
  if (
    isWrappedTool(tool) ||
    !tool.function ||
...
function getToolCallId(context: unknown): string | undefined {
  const toolContext = context as OpenRouterAgentToolTurnContext | undefined;
...
export function aggregateOpenRouterChatChunks(
  chunks: OpenRouterAgentChatCompletionChunk[],
): {
  output: OpenRouterAgentChatChoice[];
...
export function aggregateOpenRouterResponseStreamEvents(
  ...
  let finalResponse: OpenRouterAgentResponse | undefined;
...
async function traceOpenRouterCallModelTurn<TResult>(args: {
  fn: () => Promise<TResult>;
  parentSpan: Span;
  request: OpenRouterAgentCallModelRequest | undefined;
  step: number;
...
function getOpenRouterResolvedRequest(
  ...
  if (isObject(result.resolvedRequest)) {
    return result.resolvedRequest as OpenRouterAgentCallModelRequest;
  }
...
export function registerOpenRouterAgentInstrumentation(): void {
  openRouterAgentInstrumentationConsumer ??=
    new OpenRouterAgentInstrumentationConsumer();
  openRouterAgentInstrumentationConsumer.register();
}
