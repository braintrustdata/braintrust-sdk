...
async def test_adk_input_schema_serialization(memory_logger):
    ...
    # Assert complete input structure (live_connect_config varies between ADK versions)
    assert llm_span["input"] == {
        "model": "gemini-2.0-flash",
        "contents": [
            {
        ...
        "live_connect_config": ANY,  # Structure varies between ADK versions
    }

    # Assert output has the expected core fields (additional fields may vary between ADK versions)
    assert "content" in llm_span["output"]
    assert llm_span["output"]["content"]["role"] == "model"
    assert "parts" in llm_span["output"]["content"]
    assert "finish_reason" in llm_span["output"]
    assert "usage_metadata" in llm_span["output"]
