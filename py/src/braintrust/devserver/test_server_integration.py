...
def test_eval_with_dataset_id_completes_successfully(client, api_key, org_name):
    ...
    with non-null experimentName and experimentUrl.

    This reproduces the actual bug: UI sends dataset_id, Python eval fails with null
    experiment fields, causing UI to show "streaming..." indefinitely.

    THIS TEST SHOULD FAIL until we fix the dataset_id handling.
    """
    response = client.post(
        "/eval",
        headers={
        ...
        json={
            ...
            "stream": True,
            "data": {
                "dataset_id": "f1423e15-4b70-4c62-9760-42c61eea29d9",
            },
        },
    )
    ...
    assert summary_data.get("experimentName") is not None
    assert summary_data.get("experimentUrl") is not None
    assert summary_data.get("projectName") == "simple-math-eval"
