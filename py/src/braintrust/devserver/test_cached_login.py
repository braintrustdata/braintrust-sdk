...
from braintrust.devserver import server


class TestCachedLogin(unittest.TestCase):
    def setUp(self):
        """Clear the cache before each test."""
        server._login_cache = server.LRUCache(max_size=32)

    @patch('braintrust.devserver.server.login_to_state')
    def test_cached_login_caches_results(self, mock_login):
        ...
        result1 = asyncio.run(server.cached_login("api_key_1", "https://app.braintrust.com"))
        self.assertEqual(result1, mock_state)
        self.assertEqual(mock_login.call_count, 1)

        # Second call with same parameters should use cache
        result2 = asyncio.run(server.cached_login("api_key_1", "https://app.braintrust.com"))
    ...
    def test_cached_login_different_keys(self, mock_login):
        ...
        # Different API keys
        result1 = asyncio.run(server.cached_login("api_key_1", "https://app.braintrust.com"))
        result2 = asyncio.run(server.cached_login("api_key_2", "https://app.braintrust.com"))
        ...
        # Different org_name
        result3 = asyncio.run(server.cached_login("api_key_1", "https://app.braintrust.com", org_name="org1"))
    ...
    def test_cached_login_with_org_name(self, mock_login):
        ...
        # Call with org_name
        result1 = asyncio.run(server.cached_login("api_key_1", "https://app.braintrust.com", org_name="test_org"))
        self.assertEqual(result1, mock_state)
        self.assertEqual(mock_login.call_count, 1)

        # Same call should use cache
        result2 = asyncio.run(server.cached_login("api_key_1", "https://app.braintrust.com", org_name="test_org"))
        self.assertEqual(result2, mock_state)
        self.assertEqual(mock_login.call_count, 1)

        # Different org_name should not use cache
        result3 = asyncio.run(server.cached_login("api_key_1", "https://app.braintrust.com", org_name="other_org"))
        self.assertEqual(mock_login.call_count, 2)
    ...
    def test_cached_login_propagates_exceptions(self, mock_login):
        ...
        with self.assertRaises(ValueError) as cm:
            asyncio.run(server.cached_login("bad_key", "https://app.braintrust.com"))
