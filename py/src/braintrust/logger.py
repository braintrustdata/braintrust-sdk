...
class _JinjaSafeDict:
    ...
    This class implements a minimal interface that jinja2 needs, while
    preventing access to any dict methods that could cause confusion.
    """

    def __init__(self, data: Dict[str, Any]):
        # Store only the wrapped values, not the original dict
        self._wrapped = {}
        for key, value in data.items():
            self._wrapped[str(key)] = self._wrap_value(value)

    def _wrap_value(self, value: Any) -> Any:
        """Recursively wrap dicts and lists to prevent method access."""
        if isinstance(value, dict):
            return _JinjaSafeDict(value)
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        # Check _wrapped first to avoid any method access
        if hasattr(self, '_wrapped') and name in self._wrapped:
            return self._wrapped[name]
        # If not found, raise AttributeError (jinja2 will handle this as undefined)
        # This prevents jinja2 from falling back to accessing object methods
    ...
    # Explicitly do NOT define keys(), values(), items(), get() methods
    # to prevent jinja2 from accessing them as methods
    # If jinja2 needs these, it should access the data directly
...
def render_jinja(template: str, data: Any, *, strict: bool = False) -> str:
    """
    Render a template using Jinja2 with sandboxed environment for security.

    Uses SandboxedEnvironment to prevent Server-Side Template Injection (SSTI)
    attacks when rendering untrusted templates from user input.
    ...
    # Add custom filter to escape non-string values as JSON
    ...
    try:
        ...
        # Handle data similar to JS SDK - if it's a dict, spread it; otherwise use as input
        if isinstance(data, dict):
            # Wrap all dicts to prevent jinja2 from accessing dict methods
            # This ensures jinja only sees data values, not builtin methods
            wrapped_data = _JinjaSafeDict(data)
            # Create variables dict - wrap nested dicts recursively
            variables = {}
            for key, value in data.items():
                # Wrap dict values and nested structures
                if isinstance(value, dict):
                    variables[str(key)] = _JinjaSafeDict(value)
                elif isinstance(value, list):
                    variables[str(key)] = [
                        _JinjaSafeDict(item) if isinstance(item, dict) else item for item in value
                    ]
                else:
                    variables[str(key)] = value
            variables["input"] = wrapped_data
        else:
            variables = {"input": data}
        # Render with keyword arguments - jinja2 will use these as template variables
        # The _JinjaSafeDict wrapper ensures jinja only sees data, not methods
