...
describe(
  ...
  () => {
    ...
    test("processor automatically detects parent span from currentSpan() when wrapped with wrapTraced", async () => {
      // This tests that OpenAIAgentsTraceProcessor can automatically detect
      // the parent span using currentSpan() when executed within a traced context
      // We test this manually without calling the actual OpenAI SDK to avoid AsyncLocalStorage issues
      assert.lengthOf(await backgroundLogger.drain(), 0);
      ...
      const testFunction = wrapTraced(
        async (testInput: string) => {
          ...
          // Create processor WITHOUT explicitly passing parentSpan
          // It should automatically detect the parent span using currentSpan()
          const processor = new OpenAIAgentsTraceProcessor({
            logger: _logger as any,
            // Note: No parentSpan provided - should auto-detect from currentSpan()
          });

          // Manually simulate what the OpenAI Agents SDK would do
          // Create a mock trace that would be generated by the OpenAI SDK
          const mockTrace = {
            traceId: "mock-auto-detect-trace",
            name: "Agent workflow",
            metadata: { agent_name: "auto-detect-agent" },
          } as any;

          const mockSpan = {
            spanId: "mock-agent-span",
            traceId: mockTrace.traceId,
            parentId: null, // This would be the root span in the OpenAI trace
            spanData: {
              type: "agent",
              name: "Agent workflow",
              input: testInput,
          ...
          // Simulate the trace lifecycle that OpenAI SDK would trigger
        ...
        { name: "auto_detect_parent_test" },
      );

      // Execute the wrapped function
      const result = await testFunction("Test input for auto-detection");
      assert.ok(result, "Test function should return a result");
      ...
      // Find parent and child spans
      const parentSpan = spans.find(
        (s: any) => s.span_attributes?.name === "auto_detect_parent_test",
      );
      // Find the FIRST Agent workflow span, which should be the direct child of the parent
      const agentWorkflowSpans = spans.filter(
        (s: any) => s.span_attributes?.name === "Agent workflow",
      );
      assert.isTrue(
        agentWorkflowSpans.length >= 1,
        "Should have at least one Agent workflow span",
      );

      // Find the span that's a direct child of the parent span
      const childSpan = agentWorkflowSpans.find((s: any) =>
        (s as any).span_parents?.includes((parentSpan as any).span_id),
      );

      assert.ok(
        parentSpan,
        "Should find parent span with name 'auto_detect_parent_test'",
      );
      assert.ok(childSpan, "Should find child span with name 'Agent workflow'");
      ...
      if (childSpan && parentSpan) {
        ...
        assert.isTrue(
          childSpanParents.includes(parentSpanId),
          "Child span should include parent span_id in its span_parents array (auto-detected parent)",
        );
      ...
      // Verify the child span is NOT a root span
      // In Braintrust, a root span has no span_parents or an empty span_parents array
      const childSpanParents = (childSpan as any).span_parents || [];
      const isRootSpan =
        !Array.isArray(childSpanParents) || childSpanParents.length === 0;
      assert.isFalse(
        isRootSpan,
        "Agent workflow should NOT be a root span when parent is auto-detected",
      );

      // Verify input/output are captured correctly on parent span
      assert.ok(
        (parentSpan as any).input,
        "Parent span should have input logged",
      );
